<div id="szkoly_kontener">
    ...
    <?php 
        ...
        Oceny za semestr: 
        <form action="'.$this->dolacz.'wykaz_ocen.html" method="Post">
        <select name="data">
        <option value="null">wybierz semestr</option>';
            foreach($this->getOpcje()['wyswietlsemestry'] as $semestrdane){
            if($semestrdane['Zakres_Semestr'] == $_SESSION['uczenoceny'][0].'/'.$_SESSION['uczenoceny'][1]){
                $opcja = 'selected';
            } 
            else{
                $opcja = '';
